 ...
 * Constructor
 */
AS5048A::AS5048A(byte cs, bool debug /*=false*/) : _cs(cs), errorFlag(false), ocfFlag(false), position(0), debug(debug)
{
}
 ...
 */
void AS5048A::begin(uint8_t delay_micros)
{
...
uint16_t AS5048A::read(uint16_t registerAddress)
{
    ...
    delayMicroseconds(this->chip_delay);
 ...
 */
void AS5048A::setDelay(uint8_t delay_micros)
{
    ...
    if (this->debug)
    {
        Serial.println("AS5048A working with Teensy 4.0 or 4.1");
    }
#else
    this->chip_delay = delay_micros;
    if (this->debug)
    {
